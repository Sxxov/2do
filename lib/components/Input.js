...
export class Input extends X {
	...
	static #formToInputCount = new WeakMap();
	...
	connectedCallback() {
		...
		if (this.#internals.form) {
			...
			adapt: {
				const count =
					Input.#formToInputCount.get(this.#internals.form) ?? 0;
				...
				Input.#formToInputCount.set(this.#internals.form, count + 1);
			}
		}
	}
	...
	disconnectedCallback() {
		...
		adapt: {
			if (this.#internals.form) {
				...
				Input.#formToInputCount.set(this.#internals.form, 0);
			}
		}
	}
	...
	static #onFormData = (/** @type {FormDataEvent} */ e) => {
		this.funnelChildrenValuesInto(
	...
	static funnelChildrenValuesInto(
		...
		for (const { name, value, type, checked } of inputs)
			if (type === 'checkbox') into.append(name, checked ? 'on' : '');
			else into.append(name, value);
	}
